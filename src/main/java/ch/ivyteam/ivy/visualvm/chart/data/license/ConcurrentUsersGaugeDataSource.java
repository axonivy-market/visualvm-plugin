...
public class ConcurrentUsersGaugeDataSource extends GaugeDataSource {
  ...
  public ConcurrentUsersGaugeDataSource(IDataBeanProvider dataBeanProvider, int serverSessionsLimit) {
    ...
    Section greenSection = new Section(0, Math.round(serverSessionsLimit * 0.9), new Color(110, 184, 37));
    Section orangeSection = new Section(Math.round(serverSessionsLimit * 0.9), serverSessionsLimit,
            new Color(255, 210, 10));
    Section redSection = new Section(serverSessionsLimit, Math.round(serverSessionsLimit * 1.5),
            new Color(240, 40, 40));
